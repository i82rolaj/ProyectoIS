...
Tratamiento::Tratamiento(string nombrepaciente_t,int en_curso,string diagnostico,string medicacion,string fecha_inicio,string fecha_fin,string observaciones)
{
...
bool Tratamiento::anadir_tratamiento()
{
		...
		else
		{
			...
			f<<nombrepaciente_t_<<","<<en_curso_<<","<<diagnostico_<<","<<medicacion_<<","<<fecha_inicio_<<","<<fecha_fin_<<","<<observaciones_<<endl;
			f.close();
	...
	else
	{
		...
		else
		{
			while(viejo.getline(NOMPT,30,','))
			{
				...
				if(string(NOMPT).compare(nombrep)==0)
				{
					int aux=atoi(ENC);
					cout<<"valor: "<<aux<<endl;
					if(atoi(ENC)==0)
					{
					cout<<"ERROR, el tratamiento no está en curso y no puede ser modificado"<<endl;
					return false;
					}
					else
					{
						nuevo<<nombrep<<","<<en_curso_<<","<<diagnostico_<<","<<medicacion_<<","<<fecha_inicio_<<","<<fecha_fin_<<","<<observaciones_<<endl;

					}
				}
				else
				{
				nuevo<<string(NOMPT)<<","<<string(ENC)<<","<<string(DIAG)<<","<<string(MED)<<","<<string(INI)<<","<<string(FIN)<<","<<string(OBS)<<"\n";
				}
			}
			viejo.close();
			nuevo.close();
